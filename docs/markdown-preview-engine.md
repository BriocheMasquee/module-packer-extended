...
- **Inline spell/item/monster blocks**: a fenced ` ```spell `/` ```item `/` ```monster ` YAML block renders as a `.compendium-block` (spell/item) or `.statblock` (monster) matching EncounterPlus's real card — see [Compendium](Compendium) for the full field reference. Every setting that affects them (`mpx.defaultMeasurement`, `mpx.contentLanguage`, the five `mpx.defaultShowSpell*`, three `mpx.defaultShowItem*`, and four `mpx.defaultShowMonster*` toggles) is re-resolved on every render and the open preview refreshes itself automatically when any of them changes — no reload needed, unlike the theme-switching limitation below. A block's own `image`/`token` field (e.g. `spells/x.png`, `monsters/y.png`) gets the same `../` preview-only path adjustment as a page's `images/...` paths, for the same reason (the resource folder is relative to the project root, one level above a page file in `pages/`).
