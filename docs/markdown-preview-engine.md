...
- **Inline spell/item blocks**: a fenced ` ```spell `/` ```item ` YAML block renders as a `.compendium-block` matching EncounterPlus's real card — see [Compendium](Compendium) for the full field reference. Every setting that affects them (`mpx.defaultMeasurement`, `mpx.contentLanguage`, the five `mpx.defaultShowSpell*` toggles, the three `mpx.defaultShowItem*` toggles) is re-resolved on every render and the open preview refreshes itself automatically when any of them changes — no reload needed, unlike the theme-switching limitation below.
